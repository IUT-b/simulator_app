...
def interest(df,periods):    
    ...
    for period in periods:
        # トータル
        df[f'{period} year(s) total return']=df['Close'].shift(-period*12)/df['Close']
        # 年率
        df[f'{period} year(s) return']=df[f'{period} year(s) total return']**(1/period)-1
    # 積立投資の利率
    for period in periods:
        y=0
        for i in range(0,period*12):
            y=y+1/df['Close'].shift(-i)
        df[f'{period} year(s) total return2']=y*df['Close'].shift(-period*12)/12/period
        df[f'{period} year(s) return2']=(df[f'{period} year(s) total return2'])**(1/period)-1
    return df
...
def sim_goal(df,saving,value0,cash0,income,goal):
    ...
    value_max=float(dfs[INDEX_MAXs[0]]['value'].tail(1)+dfs[INDEX_MAXs[0]]['cash'].tail(1))
    ...
    value_min=float(dfs[INDEX_MINs[0]]['value'].tail(1)+dfs[INDEX_MINs[0]]['cash'].tail(1))
...
def sim_invest(df,start,end):
    ...
    df.loc[start:end,'purchases']=df.loc[start:end,'purchases']/max(df['total_purchases'])
    df['total_purchases']=df['purchases'].cumsum()
...
def sim_dissaving(df,start,r,n):
    ...
    if n==0:
        for i in range(0,int(len(df)/12)-1):
            sim.append(constant_rate(df.copy(),start + relativedelta.relativedelta(years=i),r))  
    # 定額取り崩し
    if n==1:
        for i in range(0,int(len(df)/12)-1):
            sim.append(constant_amount(df.copy(),start + relativedelta.relativedelta(years=i),r))
    # 成功率
    ...
    success=pd.DataFrame(sim_sort_bool1.sum(axis=1)/(sim_sort_bool0.sum(axis=1)+sim_sort_bool1.sum(axis=1)))
    return sim,success
...
def sim_dissaving_dash(df):
    ...
    success=pd.DataFrame((len(df_sort.columns)-df_sort_bool1.sum(axis=1))/len(df_sort.columns))
    return success
...
def fig_interest_dispersion(periods,df1,*df2):
    fig=go.Figure()
    fig.update_layout(xaxis=dict(title='投資年数'),
                      yaxis=dict(title='年利',tickformat='0%',range=[-0.3,0.3]),
                      boxmode='group',
                      )
    fig.add_trace(go.Box(x=tr(df1,periods,'return')[:,1],
                         y=tr(df1,periods,'return')[:,0],
                         name='Case1'
                        )
                 )
    for df2 in df2:
        fig.add_trace(go.Box(x=tr(df2,periods,'return')[:,1],
...
def fig_total_interest_dispersion(periods,df1,*df2):
    ...
    fig.add_trace(go.Box(x=tr(df1,periods,'total return')[:,1],
                         y=tr(df1,periods,'total return')[:,0],
                         name='Case1'
                        )
                 )
    for df2 in df2:
        fig.add_trace(go.Box(x=tr(df2,periods,'total return')[:,1],
...
def fig_sim_goal(df):
    fig=go.Figure()
    fig.update_layout(xaxis=dict(title='経過年'),
                      yaxis=dict(title='資産'),
                      barmode="stack",
                      )
...
def fig_sim_dispersion(df1,*df2):  
    ...
    fig.add_trace(go.Box(x=df_sort1.index,
                         y=df_sort1.iloc[:,0],
                         name='portfolio #1'
                         ),
                 )
    ...
    for df2 in df2:
        ...
        fig.add_trace(go.Box(x=df_sort2.index,
                             y=df_sort2.iloc[:,0],
                             name='portfolio #2'
                            ),
                      )
...
def fig_sim_invest(df,sim_invest):
    ...
    fig.add_trace(go.Scatter(x=sim_invest.index,
...
def fig_gdp(df,df_gdp):
    ...
    fig=go.Figure()
    fig= make_subplots(specs=[[{"secondary_y": True}]])
    fig.update_layout(xaxis=dict(title='年'),
                      yaxis1=dict(title='増減',type='log',tickformat='0%'),
    ...
    fig.add_trace(go.Scatter(x=df_gdp.index,
                             y=df_gdp['Close'],
                             mode='lines',
                             name='GDP'
                             ),
    ...
    fig.add_trace(go.Bar(x=df_ratio.index,
                         y=df_ratio['Close'],
                         name='株価/GDP',
                         width=0.5,
                             ),
